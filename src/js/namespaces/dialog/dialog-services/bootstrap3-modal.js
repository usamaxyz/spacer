...
function bootstrap3Modal(options) {
    ...
    if (options.confirmBtn.text)
        modalJq.on('click', '#_spa-dialog-confirm', function () {
            ...
            confirmed = true;
            if (options.input) {
                if (!validationUnit.validate(modalJq))
                    return;
                inputValue = form.getFormInputsAsObject(modalJq);
            }
    ...
    let dialog = {
        ...
        showLoading: function () {
            if (confirmBtn.length)
                dom.setLoadingButton(confirmBtn);
        },
        removeLoading: function () {
            if (confirmBtn.length)
                dom.unsetLoadingButton(confirmBtn);
        }
    };
...
export default bootstrap3Modal